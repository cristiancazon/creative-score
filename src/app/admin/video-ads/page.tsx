    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-bold text-white">Video Ads</h1>
                <Link
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-gray-400">
                        <thead className="bg-gray-950 text-gray-400 uppercase text-xs">
                            <tr>
                                <th className="px-6 py-4 font-semibold tracking-wider">Video File</th>
                        ...
                        <tbody className="divide-y divide-gray-800">
                            {loading ? (
                                ...
                                <tr><td colSpan={3} className="px-6 py-8 text-center">No video ads found.</td></tr>
                            ) : (
                                ads.map((ad) => (
                                    <tr key={ad.id} className="hover:bg-gray-800/50 transition-colors">
                                        <td className="px-6 py-4 text-white font-medium flex items-center gap-3">
                                            ...
                                            <span className="truncate max-w-sm text-xs opacity-50 px-2 py-1 bg-gray-800 rounded font-mono">{ad.video}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                            {typeof ad.match === 'object' && ad.match && (ad.match as any).home_team ? 
                                                `${((ad.match as any).home_team as any).name} vs ${((ad.match as any).away_team as any).name}` : 
