...
import { VideoAd } from '@/types/directus';

export default function VideoAdsList() {
    ...
    const fetchAds = async () => {
        try {
            const data = await directus.request(readItems('video_ads' as any, {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-extrabold text-white tracking-tight">Video Ads</h1>
                <Link
                    href="/admin/video-ads/new"
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-slate-300">
                        <thead className="bg-[#0e1726]/60 border-b border-cyan-500/10 text-slate-400 uppercase text-xs tracking-wider">
                            <tr>
                                <th className="px-6 py-4 font-bold">Video File</th>
                        ...
                        <tbody className="divide-y divide-cyan-500/5">
                            {loading ? (
                                ...
                                <tr><td colSpan={3} className="px-6 py-12 text-center text-slate-400">No video ads found.</td></tr>
                            ) : (
                                ads.map((ad) => (
                                    <tr key={ad.id} className="hover:bg-slate-800/30 transition-colors">
                                        <td className="px-6 py-4 text-white font-semibold flex items-center gap-3">
                                            ...
                                            <span className="truncate max-w-sm text-xs px-2 py-1 bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 rounded-lg font-mono" title={ad.video as string}>{ad.video as string}</span>
                                        </td>
                                        <td className="px-6 py-4 text-slate-400 font-medium">
                                            {typeof ad.match === 'object' && ad.match && (ad.match as any).home_team ? 
                                                <div className="flex items-center gap-1">
                                                    <span className="text-cyan-400">{((ad.match as any).home_team as any).name}</span> <span className="text-slate-500 text-xs">vs</span> <span className="text-cyan-400">{((ad.match as any).away_team as any).name}</span>
                                                </div> : 
                                                <span className="text-slate-600 italic text-sm">No match assigned</span>}
                                        </td>
                                        <td className="px-6 py-4 flex justify-end gap-2">
                                            <Link
                                                href={`/admin/video-ads/${ad.id}`}
